...
| **Planning item** | yoke-project/meta-yoke#2 |

## meta-yoke:continuous-integration.01 — every proposed change runs the verbs, and nothing else does work
...
## meta-yoke:continuous-integration.02 — what a run needs is declared, not taken from the runner
...
| **Expected** | every job names a pinned runner image and never a moving one, and the runner are installed at a stated version by the workflow itself — nothing a check needs is inherited from whatever an image happened to carry |
