...
| **Planning item** | yoke-project/meta-yoke#3 |

## meta-yoke:planning-status.01 — the workflow runs on the work, and runs nothing of its own
...
## meta-yoke:planning-status.02 — it is granted nothing, and with no credential it does nothing
...
## meta-yoke:planning-status.03 — an item moves forward, and never back
...
## meta-yoke:planning-status.04 — the items are read from the change, and never invented
...
## meta-yoke:planning-status.05 — a query that fails moves nothing, and says so
