...
# std: meta-yoke:planning-status.01
...
# std: meta-yoke:planning-status.02
check_granted_nothing_and_silent_without_a_credential() {
  ...
  local out status
  out="$(GH_TOKEN="" GITHUB_REPOSITORY=yoke-project/meta-yoke "$planning_script" push 2>&1)"
  status=$?
...
# std: meta-yoke:planning-status.03
...
# std: meta-yoke:planning-status.04
  ...
  cat > "$event" <<'JSON'
{
  "commits": [
    ...
    { "message": "Follow it in the layer\n\nPart of yoke-project/yoke#28" },
    { "message": "Raise the floor to 1.58.0, and nothing else" },
    { "message": "A commit naming no item at all" }
  ]
}
JSON
  named="$(items_named_by_push "$event" "yoke-project/meta-yoke")"
  rm -rf "$tmp"

  local want
  want="$(printf '%s
' "yoke-project/yoke#28" "yoke-project/meta-yoke#3" | sort)"
  [[ "$(sort <<<"$named")" == "$want" ]] || { echo "it read: $(tr '\n' ' ' <<<"$named")"; return 1; }
}

# std: meta-yoke:planning-status.05
...
SH
  ...
  out="$(PATH="$tmp:$PATH" GH_TOKEN=a-credential PLANNING_ORG=yoke-project PLANNING_PROJECT=1 \
    GITHUB_REPOSITORY=yoke-project/meta-yoke GITHUB_EVENT_PATH="$tmp/event.json" \
