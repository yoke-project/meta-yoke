...
# std: meta-yoke:the-record-of-a-run.01
...
# std: meta-yoke:the-record-of-a-run.02
check_the_record_is_assembled_from_them() {
  ...
  local tmp written
  tmp="$(mktemp -d)"
  record_fixture "$tmp" pass "meta-yoke:verbs-and-licence.01"
...
if record["repository"] != "meta-yoke":
...
# std: meta-yoke:the-record-of-a-run.03
check_a_failing_run_is_recorded_as_one() {
  ...
  local tmp written
  tmp="$(mktemp -d)"
  record_fixture "$tmp" fail "meta-yoke:verbs-and-licence.01"
...
# std: meta-yoke:the-record-of-a-run.04
...
# std: meta-yoke:the-record-of-a-run.05
